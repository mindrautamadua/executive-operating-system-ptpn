    ...
    >
      ...
      <div className="mt-2 grid min-h-0 flex-1 grid-cols-4 grid-rows-2 gap-2">
        {rows.map((r) => (
          <div
            key={r.id}
            className="flex min-w-0 flex-col overflow-hidden rounded-lg border border-[#f3e3c3] bg-[#fdf9f0] px-2.5 py-2"
          >
            ...
            <p className="mt-1.5 line-clamp-2 text-[8.5px] leading-snug text-ink-500" title={r.rootCause}>
              <span className="font-bold text-ink-700">Akar masalah: </span>
              {r.rootCause}
            </p>
            <p
              className="mt-auto flex items-start gap-1 pt-1.5 text-[8.5px] leading-snug text-ink-700"
              title={r.recovery}
            >
